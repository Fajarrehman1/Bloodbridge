...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    return res.json({ message: 'OTP generated successfully', otp });
  } catch (err) {
    return res.status(500).json({ message: err.message });
  }
});
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    // DO NOT clear OTP here — needed for reset-password step
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    console.log('reset-password called:', { email, otp: otp ? '***' : 'MISSING', newPassword: newPassword ? '***' : 'MISSING' });
    ...
    console.log('DB OTP:', user.resetOTP, '| Submitted OTP:', otp.toString().trim());
...
router.get('/match-donors', async (req, res) => {
  try {
    ...
    // Fix URL encoding — + becomes space in URL
    bloodGroup = bloodGroup.trim().replace(' ', '+');

    console.log('Searching bloodGroup:', bloodGroup);
    ...
    if (!compatible) {
      return res.status(400).json({
        message: `Invalid blood group "${bloodGroup}". Use: A+, A-, B+, B-, AB+, AB-, O+, O-`
      });
    }
    ...
    donors.forEach(donor => {
      if (!grouped[donor.bloodGroup]) {
        grouped[donor.bloodGroup] = [];
      }
